...
import { syncCartToFirestore, getCartFromFirestore, deleteItemFromFirestore, } from '../../utils/cartSync';
import { auth } from '../../firebase/FirebaseConfig';
import { onAuthStateChanged } from "firebase/auth";

const HomePageProductCard = () => {   // file 1
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, (user) => {
            if (!user) {
                // No user is logged in, clear cart data
                dispatch(clearCart());
                setFirestoreCartIds([]);
            }
        });
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {getAllProduct.slice(0, 8).map((item, index) => {
                            ...
                            return (
                                <div key={index} >
                                    <div
                                        className="group h-full border border-gray-300 rounded-xl overflow-hidden shadow-[inset_0_0_2px_rgba(0,0,0,0.6)] hover:shadow-[0_8px_30px_rgba(168,85,247,0.6)] transition duration-300 bg-white"
                                    >
